  ...
  <div class="modal-dialog" role="document">
	...
	function build_emps_table(result){
		...
		$.each(emps,function(index,item){
			...
			var vtId = $("<td></td>").append(item.vtId );
			var vtName = $("<td></td>").append(item.vtName);
			var vtPhone= $("<td></td>").append(item.vtPhone);
			var vtDate= $("<td></td>").append(getHoursTime(item.vtDate));
			var vtBegindate= $("<td></td>").append(getHoursTime(item.vtBegindat));
			var vtLeavedate= $("<td></td>").append(getHoursTime(item.vtLeavedate));
		
			var vtBlacklist = $("<td></td>").append(item.vtBlacklist);
			var attvtMeet = $("<td></td>").append(item.attvtMeet);
			...
			*/
			...
			editBtn.attr("edit-id",item.vtId );
			var delBtn =  $("<button></button>").addClass("btn btn-danger btn-sm delete_btn")
							.append($("<span></span>").addClass("glyphicon glyphicon-trash")).append("删除");
			//为删除按钮添加一个自定义的属性来表示当前删除的员工id
			delBtn.attr("del-id",item.vtId);
	...
	$(document).on("click",".delete_btn",function(){
		//1、弹出是否确认删除对话框
		var vtName = $(this).parents("tr").find("td:eq(2)").text();
		var vtId = $(this).attr("del-id");
		//alert($(this).parents("tr").find("td:eq(1)").text());
		alert(vtId);
		if(confirm("确认删除【"+vtName +"】吗？")){
			//确认，发送ajax请求删除即可
			$.ajax({
				url:"${APP_PATH}/visitor/"+vtId,
				type:"DELETE",
				success:function(result){
					alert(result.msg);
					//回到本页
	...
	$("#emp_delete_all_btn").click(function(){
		...
		$.each($(".check_item:checked"),function(){
			//this
			vtNames += $(this).parents("tr").find("td:eq(2)").text()+",";
			//组装员工id字符串
		...
		alert(del_idstr);
		if(confirm("确认删除【"+vtNames+"】吗？")){
			//发送ajax请求删除
			$.ajax({
				url:"${APP_PATH}/visitor/"+del_idstr,
				type:"DELETE",
				success:function(result){
					alert(result.msg);
					//回到当前页面
